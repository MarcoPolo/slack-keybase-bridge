...
pub fn get_bridge_info() -> BridgeInfo {
  let bridge_config_path = env!("BRIDGE_CONFIG");
  // let bridge_config_path = if let Some(p) = option_env!("BRIDGE_CONFIG") {
  //   p
  // } else {
  ...
  //   println!("e.g.: `BRIDGE_CONFIG=/keybase/private/marcopolo,cryptic_msngr/bridge_config.toml cargo run`");
  //   panic!("Missing BRIDGE_CONFIG environment variable");
  // };
  println!("Path is {}", bridge_config_path);
  let mut file = File::open(bridge_config_path)
    .expect("Missing bridge_config.toml");
